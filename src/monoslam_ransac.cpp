...
public:
  ImageConverter(char *file = NULL, char *imageTopic = "/camera/image_raw")
    : it_(nh_), first(1), slam(file)
  {
	  v_speed_ = w_speed_ = 100;
  	poses.header.frame_id = "/world"; 
    image_pub_ = it_.advertise("/monoslam/imgproc", 1);
    image_sub_ = it_.subscribe(imageTopic, 1, &ImageConverter::imageCb, this);
...
int main(int argc, char** argv)
{
	...
	std::cout << "Starting monoslam_exec" << endl;
  	...
  	ImageConverter ic(file, "/wide_stereo/left/image_rect_throttle");
  ros::spin();
  return 0;
}
